...
JustWeight <- (NewWomen$weight)
JustHeight <- (NewWomen$height)
...
HeightandWeight <- data.frame(JustHeight,JustWeight)
...
subset(HeightandWeight, JustHeight>65)
...
ggplot2(NewWomen, aes(x=height, y=weight)) + geom_point()

#Optional 'Just for fun' TASK: Change the arguments for the geom point and see what happens to the plot
ggplot2(NewWomen, aes(x=height, y=weight)) +  geom_point(shape = 21, colour = "black", fill = "white", size = 5, stroke = 5)
